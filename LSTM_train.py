...
net = Net()
net.train()
print('net.hold_last_max：'+str(net.hold_last_max))
#####
...
file_name_list=['v9888.DCE_60s','m9888.DCE_60s','rb888_1m','c9888.DCE_60s','TA888.CZCE_60s','c9888_1m']
cont_data=0
file_name=file_name_list[cont_data]
if '10s' in file_name:
    min_per_bar=1/6#每个bar是几分钟。（与每天几手的计算相关）
elif '60s' in file_name:
    min_per_bar=1#每个bar是几分钟。（与每天几手的计算相关）
hand_per_day=(0.2,5)#每天几手，(每天不超过5手，每天要多于0.2手)
...
def training_data(file_name):
    ...
    '加载cuda'
    data0=cuda(data0)
    global zoom,cost_train,cost_real,dtype,labels,labels_test,input,input_test,day_max
    dtype=data0[0].dtype
    if 'v9888' in file_name:
        zoom=5000 ;
        cost_real=1.26/zoom;
    elif 'i9888' in file_name:
        zoom=1000 ;
        cost_real=0.6/zoom;
    elif 'a9888' in file_name:
        zoom=5000 ;
        cost_real=1.26/zoom;
    ...
    '训练cost增加倍率'
    cost_train=cost_real*2.;
    print(f'缩放倍数：{zoom}；真实费率：{cost_real*zoom}；训练费率倍数：{cost_train/cost_real}；')
    ...
    separate=[0]
    for i in range(1,len(input_0[4])):
        if input_0[4][i-1][1:-1].tolist()==[5,11,4] and input_0[4][i][1:-1].tolist()!=[5,11,4]:#5代表下午三点
            separate+=[i]
...
def test(input,labels,print_str):
    ...
    with torch.no_grad(),autocast():
        ...
        right_list_train=out_pos_sum*labels-1*cost_real*Pi_cost
        #net.Dropout_Open=1
    hand_per_day_train=torch.mean(Pi_cost)*345/min_per_bar/2#每天手数实际值
    #print(f'test每天手数：{hand_per_day_train}')
    #fc.plot(out_pos_sum,save=plot_folder_name+print_str+str(cont))#+str(cont))
    ...
    return torch.sum(right_list_train,dim=0)
...
while 1:
    ...
    if cont%10==0 and cont>1:#记录
        ...
        with torch.no_grad() ,autocast():
            ...
            #out_pos_sum_test=out_pos_sum_test/torch.max(torch.abs(out_pos_sum_test))
            #out_pos_sum_test=(2*out_pos_sum_test).to(torch.int16)
            Pi_cost_test=torch.abs(torch.diff(out_pos_sum_test,prepend=prepend))#try
            
            net.Dropout_Open=1
        #out_pos_sum=(2*out_pos_sum).to(torch.int16)
        #print(net.Drop_vect[:10])
        right_list=-1*cost_real*Pi_cost+out_pos_sum*labels
        ...
        ####
        train_lst+=[(torch.sum(out_pos_sum*labels)/torch.sum(Pi_cost)).tolist()/cost_real]#每手净利润
        test_lst+=[(torch.sum(out_pos_sum_test*labels_test)/torch.sum(Pi_cost_test)).tolist()/cost_real]#test每手利润
    ...
    if cont%10==0 and cont>1:#表现
        ...
        print(f'夏普率sharp_max: {max(sharp_ratio_list):.6f}    test成本利润率_max: {max(test_lst):.6f}')
        print (f'每天手数:{round(cost_lst[-1],5)}')
        print (f'net.sh_price:{net.sh_price}')
        print (f'net.amp:{net.amp}')
        print (f'net.amp3:{net.amp3}')
        ...
        fc.plot(np.array([train_lst,test_lst,[1]*len(train_lst)]),k=1,save=plot_folder_name+'right_per_hand')
    ...
    if 1==1:#依据混合概率
        ...
        with autocast():
            ...
            Pi_cost=torch.abs(torch.diff(out_pos_sum,prepend=prepend))#try
            right_list_train=-1*cost_train*Pi_cost+out_pos_sum*labels_sh
    ...
    if input[0].device.type=='cpu':
        loss.backward()
        #net.amp3.grad = net.amp3.grad  /1000#参数梯度放大
        optimizer.step()
    elif input[0].device.type=='cuda':
        try:
            scaler.scale(loss).backward()
            #print(f'b{net.amp3.grad}')
            #net.amp3.grad = net.amp3.grad *500#参数梯度放大
        except:
            pass
    ...
    print(f'net.amp: {net.amp.tolist()}') if cont%10==0 else None
    print(f'net.amp.grad: {net.amp.grad.tolist()}') if cont%10==0 else None
    print(f'net.amp2: {net.amp2.tolist()}') if cont%10==0 else None
    print(f'net.amp2.grad: {net.amp2.grad.tolist()}') if cont%10==0 else None
    optimizer.zero_grad()
...
save()
[torch.isnan(input[i]).any() for i in range(len(input))]
result=[]
for i in range(200):
    pr1=net.state_dict()
    pr1['amp2']=pr1['amp2']*0+i/10
    net.load_state_dict(pr1)
    result+=[test(input,labels,'训练'+str(i)).tolist()]
