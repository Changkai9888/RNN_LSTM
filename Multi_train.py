...
def training_data(file_name):
    ...
    '加载cuda'
    data0=cuda(data0)
    global zoom,cost,cost_real,dtype,labels,labels_test,input,input_test,day_max,labels_vol
    dtype=data0[0].dtype
    ...
    labels_vol=input[3][:,0]
    labels_vol=eztr.smooth(labels_vol,1000)
    return labels,labels_test,input,input_test,day_max,labels_vol
#########
...
while 1:
    ...
    if cont%10==0 and cont>1:#记录
        ...
        ####
        train_lst+=[(torch.sum(out_pos_sum*labels)/torch.sum(Pi_cost)).tolist()/cost_real]#成本利润率
        test_lst+=[(torch.sum(out_pos_sum_test*labels_test)/torch.sum(Pi_cost_test)).tolist()/cost_real]#test成本利润率
    ...
    if cont%100==0 and cont>1:#表现
        ...
        print('corrcoef：'+str(corrcoef.tolist()))
    ...
    if 1!=1:#依据采样概率
        ...
        corrcoef=torch.corrcoef(torch.cat([labels_vol[1+1000:].unsqueeze(1),net.multi_volume[1000:-1].unsqueeze(1)],dim=1).T)[0,1]#multi的交易量与实际交易量序列的相关系数
        loss-=corrcoef
        if cost_lst!=[] and min(cost_lst)<hand_per_day[1]:
    ...
    #print(f'net.amp: {net.amp.grad.tolist()}') 
    optimizer.zero_grad()
