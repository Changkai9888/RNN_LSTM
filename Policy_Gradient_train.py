...
from Policy_Gradient_net import Net
...
def training_data(file_name):
    ...
    #if data0[0].device.type=='cpu':
        #data0=[i.to(torch.float32) for i in data0]
    global cost,dtype,labels,labels_test,input,input_test
    dtype=data0[0].dtype
    if file_name in ['c9888_1m','c9888_1m_mini_1w']:     
        zoom=5000 ;cost=1.156/zoom
    if file_name=='au888_1m':
        zoom=100; cost=0.04/zoom
    if file_name=='rb888_1m':
        zoom=5000; cost=1.5/zoom
    ...
    test_part=int(len(labels_0)*0.2)
    labels,labels_test=labels_0[:-test_part-1],labels_0[-test_part:]
    input=input_conv[:-test_part-1], input_conv_neg[:-test_part-1], input_mirr[:-test_part-1], input_vol[:-test_part-1], input_onehot[:-test_part-1]
    input_test=input_conv[-test_part:], input_conv_neg[-test_part:], input_mirr[-test_part:], input_vol[-test_part:], input_onehot[-test_part:]
    return labels,labels_test,input,input_test
#########
cont_data=0
...
def test(input,labels,print_str):
    ...
    with torch.no_grad(),autocast():
        ...
        #Pi_cost=torch.abs(torch.diff(out_pos_sum,prepend=prepend))
        right_list_train=out_pos_sum*labels-1*cost*Pi_cost
        #net.train()
    hand_per_day_train=torch.mean(Pi_cost)*345/min_per_bar#每天手数实际值
...
while 1:
    ...
    if cont%10==0 and cont>1:#记录
        ...
        with torch.no_grad() ,autocast():
            #net.eval()
            output_0,out_prob=net(input,(0,1,0),2)
            Prob_cost=torch.abs(torch.diff(output_0, prepend=prepend))
            output_0_test, out_prob_test = net(input_test,(0,1,0),2)
            Prob_cost_test = torch.abs(torch.diff(output_0_test, prepend=prepend))
            #net.train()
        right_list=output_0*labels-1*cost*Prob_cost
        sharp_ratio= Return(right_list)
        sharp_ratio_list+=[sharp_ratio.tolist()]
        ####
        right_list_test=output_0_test*labels_test-1*cost*Prob_cost_test
        sharp_ratio_test= Return(right_list_test)
        sharp_ratio_list_test+=[sharp_ratio_test.tolist()]
        ####
        train_lst+=[(torch.sum(output_0*labels)/torch.sum(Prob_cost)).tolist()]#每手净利润
        test_lst+=[(torch.sum(output_0_test*labels_test)/torch.sum(Prob_cost_test)).tolist()]#test每手利润
        cost_lst+=[torch.mean(Prob_cost.to(dtype)).tolist()*345/min_per_bar]#平均每天几手
    if cont%100==0 and cont>1:#表现
        ...
        print(f'夏普率sharp_max: {max(sharp_ratio_list):.6f}    test_max: {max(test_lst):.6f}')
        print (f'每天手数:{round(cost_lst[-1],5)}')
        print (f'net.sh:{round(net.sh.tolist(),5)}')
        ...
        fc.plot(output_0,save=plot_folder_name+'pos_list')
        fc.plot(output_0_test,save=plot_folder_name+'pos_test_list')
        #fc.plot([train_lst,test_lst,[0.0004]*len(train_lst),[0.]*len(train_lst)],k=1,save=plot_folder_name+'right_per_hand')
        fc.plot(np.array([train_lst,test_lst,[cost]*len(train_lst)])/cost,k=1,save=plot_folder_name+'right_per_hand')
        fc.plot(loss_lst,save=plot_folder_name+'loss_list')
        fc.plot([sharp_ratio_list,sharp_ratio_list_test],k=1,save=plot_folder_name+'sharp_ratio_list')
        fc.plot(cost_lst[10:],save=plot_folder_name+'cost_lst_每天手数')
        fc.plot(torch.cumsum( right_list  ,dim=0),save=plot_folder_name+'right_list')#
        fc.plot(torch.cumsum(output_0_test*labels_test-1*cost*Prob_cost_test,dim=0),save=plot_folder_name+'right_list_test')
    ...
    with autocast():
        ...
        if 1!=1:#依据采样概率
            ...
            for s in range(40):
                output_0,out_prob=net(input_here,(0,1,0),2)
            ...
            if  cost_lst!=[] and min(cost_lst)<hand_per_day[1]:#等待手数自然下降后，再进行管理。
                hand_per_day_train=torch.mean(Prob_cost.to(data0[0].dtype))*345/min_per_bar
            ...
            out_pos_sum, Pi_cost, prob_value=net(input,(0,1,0),0,1)
            ...
            loss=-Return(right_list_train,mod=2)        #夏普率损失函数
            if cost_lst!=[] and min(cost_lst)<hand_per_day[1]:
                hand_per_day_train=torch.mean(Pi_cost)*345/min_per_bar#每天手数实际值
                open_decide=prob_value[:,1,0]+prob_value[:,1,2]
    ...
    if input[0].device.type=='cpu':
        ...
        '''if cont%10==0 and cont>1:
            for param in net.parameters():
                ...
                param.grad *= 0.1'''
        ####
    grad_test[0]=[net.pi_exit.bias.grad.tolist()[0] if net.pi_exit.bias.grad!=None else None]
